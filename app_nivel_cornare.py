...
def render_inicio():
    ...
    with col_foto:
        if portada:
            st.image(portada["foto"], caption=f"Estación {codigo_estacion} (foto: CORNARE)", use_container_width=True)
        else:
...
def render_niveles():
    ...
    st.altair_chart(barras + linea_media, use_container_width=True)
